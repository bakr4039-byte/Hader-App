...
// جلب قائمة المعلمين ووضعها في القائمة المنسدلة عند فتح الصفحة
async function loadTeachersList() {
    ...
    Object.keys(teachers).forEach(id => {
        ...
        opt.value = id;
        opt.innerHTML = teachers[id].name;
...
async function fetchRangeReport() {
    ...
    table.innerHTML = "<tr><td colspan='6'>جاري جلب البيانات...</td></tr>";

    let s = { abs:0, lDays:0, lMins:0, eMins:0, noOut:0 };
    ...
    try {
        ...
        for (let d = new Date(from); d <= to; d.setDate(d.getDate() + 1)) {
            ...
            Object.keys(teachers).forEach(id => {
                // الفلترة: إذا تم اختيار معلم محدد، نتجاهل البقية
                if (selectedID !== 'all' && id !== selectedID) return;
                ...
                if (!rec) {
                    s.abs++;
                    tableRows += `<tr style="color:#e74c3c"><td>${t.name}</td><td>${dateStr}</td><td colspan="4">غائب</td></tr>`;
                } else {
        ...
        table.innerHTML = tableRows || "<tr><td colspan='6'>لا توجد بيانات لهذا النطاق</td></tr>";
    ...
    } catch (e) { table.innerHTML = "<tr><td colspan='6'>خطأ في الاتصال</td></tr>"; }
}
